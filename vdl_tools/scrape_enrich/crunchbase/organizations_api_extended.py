...
default_file_paths: OutputFilePaths = {
    "organizations": wd / 'organizations.json',
    "funding_rounds": wd / 'funding_rounds.json',
    "investor_person": wd / 'investors_person.json',
    "investor_orgs": wd / 'investor_organizations.json',
    "founders": wd / 'org_founders.json'
}
...
def companies_ipo_query(organization_uuids, extra_filters=[]):
    print('Querying IPO events by company ID...')
    return __iterative_query(
        ipos.query,
        ipos.all_fields,
...
def companies_aquiree_query(organization_uuids, extra_filters=[]):
    print('Querying Acquisitions by acquiree ID...')
    return __iterative_query(
        acquisitions.query,
...
def query_companies_extended(
    items_list,
    output_files: OutputFilePaths = default_file_paths,
    ...
    pl.Path(output_files['organizations']).parent.mkdir(parents=True, exist_ok=True)
    if pl.Path.exists(output_files['organizations']) and not force_query:
        logger.info('loading organizations from %s', output_files['organizations'])
        organizations = pd.read_json(output_files['organizations'])
    else:
        ...
        # write to json instead of excel
        organizations.to_json(output_files["organizations"], orient='records')

    companies_funding_rounds: pd.DataFrame = None
    if pl.Path.exists(output_files['funding_rounds']) and not force_query:
        logger.info('loading funding rounds from %s', output_files['funding_rounds'])
        companies_funding_rounds = pd.read_json(output_files['funding_rounds'])
    else:
        ...
        # write to json instead of excel
        companies_funding_rounds.to_json(output_files["funding_rounds"], orient='records')
    ...
    if people_investors is None:
        ...
        # write to json instead of excel
        people_investors.to_json(output_files['investor_person'], orient='records')
    ...
    if org_investors is None:
        ...
        # write to json instead of excel
        org_investors.to_json(output_files['investor_orgs'], orient='records')
    ...
    if len(founder_ids) == 0:
        ...
        # write to json instead of excel
        founders.to_json(output_files['founders'], orient='records')
