...
# Artifacts `marimo export html-wasm` copies from its own _static/ that are not
# part of the app. CLAUDE.md is marimo's generic agent prompt; it must never be
# served from a client bucket.
EXPORT_JUNK = ("CLAUDE.md",)
...
def cmd_export(args) -> int:
    ...
    # Deleted here rather than only excluded at upload, so a folder synced by
    # hand (not through `deploy`) cannot ship it either.
    for name in EXPORT_JUNK:
        if (out / name).is_file():
            (out / name).unlink()
            print(f"  removed {name} (copied in by marimo, not part of the app)")
    ...
    run(
        [
            ...
            *prune,
            ...
            "--exclude",
            "public/*",
            # `export` already deletes these; excluded again for exports built
            # with plain `marimo export html-wasm`.
            *(arg for name in EXPORT_JUNK for arg in ("--exclude", name)),
            "--exclude",
            ".nojekyll",
            "--cache-control",
            IMMUTABLE,
