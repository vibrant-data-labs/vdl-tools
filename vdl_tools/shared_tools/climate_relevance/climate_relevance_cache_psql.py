...
class ClimateRelevanceCache(PromptResponseCacheSQL):
    """Cache for climate relevance predictions using fine-tuned GPT models.
    ...
    def bulk_get_climate_relevance(
        self,
        ...
        """Get climate relevance predictions for multiple texts in bulk.
    ...
    def predict_dataframe(
        ...
        """Generate predictions and add them directly to the DataFrame.
        ...
        results = self.bulk_get_climate_relevance(
            given_ids_texts=given_ids_texts,
            model=model,
        ...
        df_result['prediction'] = df_result[idn].map(lambda x: results.get(x, (None, None))[0])
        df_result['probability'] = df_result[idn].map(lambda x: results.get(x, (None, None))[1])
